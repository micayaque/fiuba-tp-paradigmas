...
public class AlternanciaDeFasesTest {
    ...
    public void laPartidaAlternaEntreNocheYDiaIncrementandoLaRondaAlVolverALaNoche() {
        ...
        assertEquals("Fase Nocturna", moderador.faseActual().descripcion());

        moderador.registrarVoto(mafioso, ciudadano1);
        moderador.avanzarFase();

        assertEquals("Fase Diurna", moderador.faseActual().descripcion(),
                "Tras resolver la noche debe pasar al día, misma ronda");
        assertEquals(1, moderador.numeroDeRonda());
        ...
        assertEquals("Fase Nocturna", moderador.faseActual().descripcion(),
                "Tras resolver el día debe volver a la noche");
        assertEquals(2, moderador.numeroDeRonda(),
                "Al volver a la noche se inicia una nueva ronda");
    }
}
