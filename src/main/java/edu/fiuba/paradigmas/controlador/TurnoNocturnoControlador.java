...
import edu.fiuba.paradigmas.vista.fase.nocturna.AccionNocturnaVista;
...
public class TurnoNocturnoControlador implements IdentificadorRol {
    ...
    public void esMafioso() {
        AccionNocturnaVista vista = new AccionNocturnaVista("Mafia", "💀", "#ff416c", "Elige a quién eliminar", nombresVivos);
    ...
    public void esMedico() {
        AccionNocturnaVista vista = new AccionNocturnaVista("Médico", "➕", "#00b894", "Elige a quién proteger", nombresVivos);
        vista.configurarAcciones(false, seleccion -> {
            try {
    ...
    public void esDetective() {
        AccionNocturnaVista vista = new AccionNocturnaVista("Detective", "🔍", "#3182ce", "Elige a quién investigar", nombresVivos);
        vista.configurarAcciones(false, seleccion -> {
            try {
                Jugador sospechoso = controlador.buscarJugadorPorNombre(seleccion);
                Memento memento = this.moderador.registrarInvestigacion(this.jugadorActivo, sospechoso);
                this.controlador.registrarEventoNocturno(memento);
                MementoDeInvestigacion inv = (MementoDeInvestigacion) this.controlador.desenvolver(memento);
